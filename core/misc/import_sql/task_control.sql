...
CREATE TABLE IF NOT EXISTS `task_control` (
  ...
  `type` enum('keep','minute','hour','day','Fnk','month') collate utf8_unicode_ci NOT NULL default 'keep' COMMENT '执行类型',
  `exec_time` varchar(255) collate utf8_unicode_ci NOT NULL COMMENT '执行时间可使用的公式变量: $year, $month, $day, $Fnkday, $hour, $minute, $second;可使用的比较符: +-*/% ==<>;',
