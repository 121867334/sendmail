...
class Ext_String {
	 ...
	 * @param string $charset 编码, 默认使用配置里的编码 
	 ...
	 * 
	 * @param string $str 字符串
	 * @param integer $level 目录层数
	 * @param integer $length 每层目录长度 
	 * @return string
	 */
	 ...
	 * @param array $assignVars 包含公式要使用的变量的数组
	 * @return mixed 计算结果
	 */
	 ...
	 * 获取随机字符
	 * 
	 * @param integer $num 字符串位数
	 * @return string 
	 */
	public static function getSalt($num = 4,$type=0) {
	    switch ($type)
	    {
			case 1:
				$str = "23456789ABCDEFGHIJKLMNPQRSTUVW";
				break;
			default:
	    ...
	    for ($i = 0; $i < $num; $i++) {
	    	switch ($type)
	    	{
	    		...
	    		case 1:
	    			...
	    			break;
	    		case 2:
	    			$rs .= iconv('gbk', 'utf-8', chr(rand(161,215)).chr(rand(161,249)));
	    			break;
	    	}
	    }
		return $rs;
	...
	public static function passHash($password) {
		return md5(md5($password).Fn::$config['encrypt_key']);
		//$hash=md5($password);
	...
	public static function getSplitOne($str)
	{
		$arr = explode('|', $str);
		$str = $arr[0];
		$arr = explode('/', $str);
	...
	public static function getUnserialize($str)
	{
		if(!$str)return false;
		$arr = unserialize($str);
		return self::getAddSlashes($arr);
	}
	public static function getAddSlashes($arr)
	{
	...
	public static function getStripslashes($arr)
	{
		if(!is_array($arr))return false;
		foreach ($arr as $k=>$v)
		{
	...
	public static function queryStringToArray($str)
	{
		if(!$str)return false;
		$arr = explode('&', $str);
		$rs = array();
	...
	public static function queryArrayToString($arr)
	{
		if(!$arr)return false;
		$rs = '';
		if($arr)
