...
class Logs {
	 ...
	 * 获取日志文件路径
	 * 
	 * @param string $logId 日志类型
	 * @param string $time 时间 
	 * @return string 日志文件路径 
	 */
	public static function getLogFile($logId, $time = null) {
		if (!$time) $time = Ext_Date::now();
		if (Fn::REQUEST_SERVER == Fn::$requestType) {
			$logPath = Fn::$config['data_path'] . 'server/' . Ext_Date::format($time, 'Ymd');
		} else {
			$logPath = Fn::$config['data_path'] . 'client/' . Ext_Date::format($time, 'Ymd');	
		}
		if (!is_dir($logPath)) {
