...
class Lang {	
	...
	private function _getLangPack() {
		$table = Fn::$config['lang_table'] . '_' . Fn::$config['lang_type'];
	 ...
	 * 获取语言包值
	 * 
	 * @param string $name 语言包键名
	 * @param array/string $args 占位符替换参数 
	 * @return string 语言包值 
	 */
	public static function get($name, $args = array()) {
		if (!isset(Fn::$box['lang'])) {
			if (Fn::$config['lang_cache']) {
				$langFile = Fn::$config['data_path'] . 'cache/' 
								.'lang_' . Fn::$config['lang_type'] . '.php';
				if (is_file($langFile)) {
					Fn::$box['lang'] = require $langFile;
				} else {
		...
		if (empty(Fn::$box['lang'][$name])) {
			return $name;	
		}
		$text = Fn::$box['lang'][$name];
		if ($args) {	
			if (!is_array($args)) {
				$args = func_get_args();
				array_shift($args);	
	 ...
	 * 设置语言包值
	 * 
	 * @param string $langName 语言包标识
	 * @param string $name 语言包键名
	 * @param string $value 语言包值
	 * @param string $package 语言包所属模块 
	 * @return integer 影响行数 
	 */
