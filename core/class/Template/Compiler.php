...
class Template_Compiler {
	...
	private $_pattern = array(
		...
		'/\{php\}/is',	//<!--{ php }-->
		'/\{\/php\}/is',	//<!--{ /php}-->
		'/\{insert_scripts\s*file=([^}]+)\}/is', // <!--{ insert_scripts ..}-->
		'/\{html_options\s+name=(.+?)\s+options=(\$[^\s]+)(\s+checked=)?([^\s]+)?(\s+type=\")?([^\s\"]+)?\"?(\s+isval=\"?)?([^\s\"]+)?\"?\s*\}/is', // <!--{ html_options ..}-->
	);
	...
	private $_replace = array(
		...
		"<?php \n",
		"\n?>",
		"<?php \$this->insert_scripts(\\1);?>",
		"<?php \$this->html_options(\\1,\\2,\\4,\"\\6\",\"\\8\");?>",
	);
		
	/**
	 * 构造方法
	 * @param string $tplDir 模板目录
	 * @param string $compileDir 模板编译目录 
	 * @return void
	 */
	public function __construct() {
		$this->_debug = Fn::$config['template_debug'];
	...
	public function compile($tplCon) {
		...
		*/
		...
		// 添加编译信息
		$tplCon = "<?php\n /* compiled by (FnPHP) at (" . date('Y-m-d H:i:s') . ") */\n?>\n" . $tplCon;
