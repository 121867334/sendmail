...
abstract class Session {		
	 ...
	 * @var integer 数据过期时间 
	 */
	protected static $_lifetime = 1440;
	...
	public static function start() {
		/*$handleClass = Fn::$config['session_handle_class'];
		if ('Session_Database' == $handleClass) {
			$db = load_db(Fn::$config['session_db_tag']);
			$handle = new Session_Database($db, Fn::$config['session_table']);
		...
		session_start();
		//self::setLifeTime();
		/*if(Ext_Network::getBrowser() != 'Internet Explorer')
		{
			self::setLifeTime();
		}*/
	}
     ...
     * 设置Session生存周期
     * 
     * @param integer $time 秒 
     * @return void 
     */
    public static function setLifeTime($time = null) {
    	$now = time();
    	if ($time > 0) $time = $now + $time;
    	elseif ($time < 0) $time = $now - $time;
    	else $time = $now + self::$_lifetime;
    	...
    	if(!$_COOKIE[self::name()])
    	{
    		...
    		{
    		setcookie(self::name(), $_COOKIE[self::name()], $time, "/");
    	}
    }
}
