...
class Db_Plus {	
	...
	public function getSqlFile($path) {
		//$arr = glob(Fn::$config['data_path'] . 'db_backup/*.sql');	
		$arr = Ext_Dir::getDirList(Fn::$config['data_path'] . 'db_backup', Ext_Dir::TYPE_FILE, array(), array('sql'));
		$sqlFile = array();
		foreach ($arr as $value) {
			$value = Fn::$config['data_path'] . 'db_backup/' . $value;
			$data = array();
	...
	public function getSamePreFile($path, $filename) {
		...
		if (preg_match("/^(\d+_\d+)_(\d+)\.sql$/i", basename($filename), $tmp)) {
			$allFiles = Ext_Dir::getDirList(Fn::$config['data_path'] . 'db_backup', Ext_Dir::TYPE_FILE, array(), array('sql'));
			if ($allFiles) {
