...
abstract class Db {	
	...
	public function table($table) {
		if (Fn::$config['db_table_prefix'] && false !== strpos($table, '#@_')) {
			$table = str_replace('#@_', Fn::$config['db_table_prefix'], $table);	
		}
	...
	protected function _dealDbName($dbName) {
		if (isset(Fn::$config['db_name_alias'][$dbName])) {
			$dbName = Fn::$config['db_name_alias'][$dbName];	
		}
		return $dbName;	
	}
	...
	protected function _checkSqlValue($value) {
		/*if(Fn::$config['db_driver']=='Db_Mysql' && function_exists('mysql_real_escape_string')){
	...
	protected function _recordSqlQuery($sql, $startTime, $endTime) {
    	...
    	// 纪录慢查询日志
    	if (true == Fn::$config['db_sql_log']) {
			if ($runTime > Fn::$config['db_slow_sql_time']) {
				$sqlInfo = array(
		    		'host' => $this->_host,
		    		'db' => $this->_dbname,
		    		'sql' => $sql,
